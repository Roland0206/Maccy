...
class ClipboardTests: XCTestCase {
  ...
  let concealedType = NSPasteboard.PasteboardType.concealed
  ...
  func testIgnoreTransientTypes() {
    assertIgnoresPrivacyMarker(transientType)
  }

  func testIgnoreConcealedTypes() {
    assertIgnoresPrivacyMarker(concealedType)
  }

  func testIgnoreAutogeneratedTypes() {
    assertIgnoresPrivacyMarker(autogeneratedType)
  }
  ...
  private func assertIgnoresPrivacyMarker(_ type: NSPasteboard.PasteboardType) {
    let hookExpectation = expectation(description: "Hook is called")
    hookExpectation.isInverted = true
    clipboard.onNewCopy({ (_: HistoryItem) in
      hookExpectation.fulfill()
